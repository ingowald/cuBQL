...
if([version]$CUDA_VERSION_FULL -ge [version]"13.0") {
    ...
    $CUDA_PACKAGES_IN += "cublas"
    $CUDA_PACKAGES_IN += "cublas_dev"
    $CUDA_PACKAGES_IN += "crt"
    $CUDA_PACKAGES_IN += "ctadvisor"
    $CUDA_PACKAGES_IN += "cuda_profiler_api"
    $CUDA_PACKAGES_IN += "cufft"
    $CUDA_PACKAGES_IN += "cufft_dev"
    $CUDA_PACKAGES_IN += "cuobjdump"
    $CUDA_PACKAGES_IN += "cupti"
    $CUDA_PACKAGES_IN += "curand"
    $CUDA_PACKAGES_IN += "curand_dev"
    $CUDA_PACKAGES_IN += "cusolver"
    $CUDA_PACKAGES_IN += "cusparse"
    $CUDA_PACKAGES_IN += "cusparse_dev"
    $CUDA_PACKAGES_IN += "cuxxfilt"
    $CUDA_PACKAGES_IN += "nsight_compute"
    $CUDA_PACKAGES_IN += "nsight_systems"
    $CUDA_PACKAGES_IN += "nsight_vse"
    $CUDA_PACKAGES_IN += "nvcc"
    $CUDA_PACKAGES_IN += "nvdisasm"
    $CUDA_PACKAGES_IN += "nvjtlink"
    $CUDA_PACKAGES_IN += "nvtx"
}
